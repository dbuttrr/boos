...
- Full-bleed Leaflet map fills the viewport; in home-screen PWA mode the status bar is translucent (`black-translucent`) so tiles can run under the notch. The watch-sheet stays at `0.75rem` from the layout bottom (unchanged). Only `.map-stage` extends past the layout viewport (`safe-area-inset-top + safe-area-inset-bottom`) so map tiles cover the grey strip under the sheet instead of moving the sheet. Fixed chrome glass lives on `::before`. Watchlist sits in a more transparent liquid-glass sheet (`.watch-sheet`, `--watch-sheet-glass`) over the bottom half, inset equally on left/right/bottom (`0.75rem`; home-indicator safe-area ignored so the bottom gutter matches the sides). Last-updated time sits at the top of the sheet with a “next Ns” countdown to auto-refresh.
