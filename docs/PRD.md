...
- Add-flow paints immediately; route catalog stays in memory (idle-prefetched). Typing shows up to 8 prefix-matched suggestions in a dropdown **above** the route input (keeps list visible above the on-screen keyboard) — no full-route datalist in the DOM.
