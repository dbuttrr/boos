...
- Full-bleed Leaflet map fills the viewport; in home-screen PWA mode the status bar is translucent (`apple-mobile-web-app-status-bar-style: black-translucent`). The map stage and sheet/top chrome keep backgrounds/`backdrop-filter` off the fixed element itself (glass on `::before`) so iOS 26 does not sample grey into system chrome. Bottom UI uses `--watch-sheet-bottom` (`inset − safe-area-inset-bottom`) and the map extends into the home-indicator band so the sheet stays on the physical screen edge instead of floating up. Watchlist sits in a more transparent liquid-glass sheet (`.watch-sheet`, `--watch-sheet-glass`) over the bottom half, inset equally on left/right/bottom (`0.75rem`; home-indicator safe-area subtracted so the gutter matches the sides on the physical screen). Last-updated time sits at the top of the sheet with a “next Ns” countdown to auto-refresh.
