...
- Circular **+** (floating above the watch-sheet, outside the sheet) starts an inline **add-flow** panel stacked above **+**: enter route → pick direction (stacked chips) → tap stop on the **main map** → confirm. **+** becomes **×** while add-flow is active (tap to cancel). No back control after route entry — cancel via **×** only. Watchlist rows are not interactive during add-flow. Success shows a short toast above the anchor. While **two or more** routes are focused, the same control becomes clear-all **×** (scale pulse + slight red tint); with zero or one focused it stays **+**.
- Add-flow paints immediately; route catalog stays in memory (idle-prefetched). Typing shows up to 8 prefix-matched suggestions in a liquid-glass dropdown **above** the route input (keeps list visible above the on-screen keyboard) — no full-route datalist in the DOM.
