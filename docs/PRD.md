...
- Full-bleed Leaflet map fills the viewport; in home-screen PWA mode the status bar is translucent (`black-translucent`) so map tiles run under the notch. Bottom UI uses `--watch-sheet-bottom` (`inset − safe-area-inset-bottom`) and the map extends into the home-indicator band so the sheet stays on the physical screen edge. Fixed chrome glass lives on `::before` so iOS 26 does not sample it into system chrome. Watchlist sits in a more transparent liquid-glass sheet (`.watch-sheet`, `--watch-sheet-glass`) over the bottom half, inset equally on left/right/bottom (`0.75rem` from the physical edge). Last-updated time sits at the top of the sheet with a “next Ns” countdown to auto-refresh.
