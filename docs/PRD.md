...
- Circular **+** (floating above the watch-sheet, outside the sheet) starts an inline **add-flow** panel stacked above **+**: enter route → pick direction (stacked chips, no back) → tap stop on the **main map** → confirm. **+** becomes **×** while add-flow is active (tap to cancel). Back chevron on picking / confirm only. Watchlist rows are not interactive during add-flow. Success shows a short toast above the anchor. While **two or more** routes are focused, the same control becomes clear-all **×** (scale pulse + slight red tint); with zero or one focused it stays **+**.
