...
- Full-bleed Leaflet map fills the viewport; on iOS Safari the map stage bleeds past the layout viewport (safe-area + toolbar guard bands) with a hidden scroll runway so Liquid Glass chrome samples map tiles instead of the page background. Watchlist sits in a more transparent liquid-glass sheet (`.watch-sheet`, `--watch-sheet-glass`) over the bottom half, inset equally on left/right/bottom (`0.75rem`; home-indicator safe-area ignored so the bottom gutter matches the sides). Last-updated time sits at the top of the sheet with a “next Ns” countdown to auto-refresh.
